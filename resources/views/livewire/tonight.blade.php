<div class="space-y-4 max-w-3xl mx-auto">
    <div class="flex items-baseline justify-between">
        <flux:heading size="xl">Tonight</flux:heading>
        <flux:text size="sm" variant="subtle">{{ $today->format('l, M j') }}</flux:text>
    </div>

    @if (! $dinner)
        <flux:card>
            <flux:heading size="lg">No dinner planned</flux:heading>
            <flux:text class="mt-1">{{ $defaultAttendeeIds->count() }} expected at the table.</flux:text>
            <div class="mt-4">
                <flux:button :href="url('/plan?date=' . $today->toDateString() . '&slot=dinner')" variant="primary">Plan tonight's dinner</flux:button>
            </div>
        </flux:card>
    @else
        @php
            $servings = max(1, (int) ($recipe?->servings ?? 1));
            $hasMacros = $perServing && $perServing['calories'] > 0;
        @endphp
        ...
        <flux:card class="overflow-hidden p-0!">
            <div class="p-5 sm:p-6 bg-gradient-to-br from-indigo-50 to-amber-50 dark:from-indigo-950/40 dark:to-amber-950/30">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <div class="text-xs uppercase tracking-wider text-indigo-700 dark:text-indigo-300 font-semibold">Dinner</div>
                        <flux:heading size="xl" class="mt-1 break-words">{{ $dinner->displayName() }}</flux:heading>
                ...
                <div class="border-t border-zinc-200 dark:border-zinc-700 p-4 sm:p-5">
                        ...
                        <div class="mb-4 pb-4 border-b border-zinc-100 dark:border-zinc-800">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <flux:button size="sm"
                                    ...
                                    wire:click="setMyStatus({{ $dinner->id }}, 'eating')">
                                    🍽 I'm eating
                                </flux:button>
                                <flux:button size="sm"
                                    :variant="$myStatus === 'running_late' ? 'primary' : 'filled'"
                                    wire:click="setMyStatus({{ $dinner->id }}, 'running_late')">
                                    ⏰ Late
                                </flux:button>
                                <flux:button size="sm"
                                    :variant="$myStatus === 'not_eating' ? 'primary' : 'filled'"
                                    wire:click="setMyStatus({{ $dinner->id }}, 'not_eating')">
                                    ✕ Skip
                                </flux:button>
                            </div>
                        </div>
                    ...
                    <div class="space-y-2">
                        @foreach ($members as $m)
                            @if ($myMember && $m->id === $myMember->id)
                                @continue
                            @endif
                            @php $s = $statuses[$m->id] ?? 'not_eating'; @endphp
                            <div class="flex items-center justify-between gap-2">
                                <div class="flex items-center gap-2 min-w-0">
                                    <x-avatar :member="$m" size="sm" />
                                    <flux:text size="sm" class="truncate">{{ $m->name }}</flux:text>
                                </div>
                                <div class="flex gap-1">
                                    <flux:button size="xs"
                                        :variant="$s === 'eating' ? 'primary' : 'ghost'"
                                        wire:click="setMemberStatus({{ $dinner->id }}, {{ $m->id }}, 'eating')">🍽</flux:button>
                                    <flux:button size="xs"
                                        :variant="$s === 'running_late' ? 'primary' : 'ghost'"
                                        wire:click="setMemberStatus({{ $dinner->id }}, {{ $m->id }}, 'running_late')">⏰</flux:button>
                                    <flux:button size="xs"
                                        :variant="$s === 'not_eating' ? 'primary' : 'ghost'"
                                        wire:click="setMemberStatus({{ $dinner->id }}, {{ $m->id }}, 'not_eating')">✕</flux:button>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @if ($dinner->notes)
                <div class="border-t border-zinc-200 dark:border-zinc-700 p-4 sm:p-5 bg-amber-50/50 dark:bg-amber-950/20">
                    ...
                    <flux:text size="sm">{{ $dinner->notes }}</flux:text>
                </div>
            @endif
        </flux:card>
    @endif
